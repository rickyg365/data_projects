...
def insert_todo(todo: Todo):
    ...
    # added context manager to commit this after running
    with conn:
        # This parameter substitution syntax prevents sql injection attacks
...
def delete_todo(position):
    ...
    # Commits after we leave context manager
    with conn:
...
def change_position(old_position: int, new_position: int, commit=True):
    c.execute('UPDATE todos SET position = :position_new WHERE position = :position_new',
    {
...
def update_todo(position: int, task: str, category: str):
    with conn:
        has_task = True if task is not None else False
        has_category = True if category is not None else False

        if has_task and has_category:
